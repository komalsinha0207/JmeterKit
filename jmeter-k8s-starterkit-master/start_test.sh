...
input_file="testplan_repo.json"
repo_url=`jq -r '.repo_url' "$input_file"`
test_plan_name=`jq -r '.test_plan_name' "$input_file"`
repo_name=$(sed -e 's:.*/::' <<< $repo_url)
repo_name=$(sed 's/.git//g' <<< $repo_name)
repo_url1=$(sed -s 's/https:\/\//https:\/\/oauth2:GITTOKVAL@/g' <<< $repo_url)
repo_url1=${repo_url1/GITTOKVAL/$GITTOKVAL}
...
exec_num=$(date +%d-%H-%M-%S)
echo "Data: ${exec_num}"
cd ./k8s/jmeter/
cat jmeter-master_template.yaml | sed -e "s/<exec_num>/$exec_num/" >> jmeter-master${exec_num}.yaml
cat jmeter-slave_template.yaml | sed -e "s/<exec_num>/${exec_num}/" >> jmeter-slave${exec_num}.yaml
cd ../../
...
kubectl -n "${namespace}" delete -f k8s/jmeter/jmeter-master${master_num_plus_one}.yaml -f k8s/jmeter/jmeter-slave${master_num_plus_one}.yaml 2> /dev/null
kubectl -n "${namespace}" apply -f k8s/jmeter/jmeter-master${master_num_plus_one}.yaml -f k8s/jmeter/jmeter-slave${master_num_plus_one}.yaml
